...
@Component({
  selector: 'app-archivos-psicologia',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-psicologia.component.html',
  styleUrl: './archivos-psicologia.component.css',
})
export class ArchivosPsicologiaComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.psicologia.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_psicologia(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.psicologia = this.jugador.psicologia.filter(
