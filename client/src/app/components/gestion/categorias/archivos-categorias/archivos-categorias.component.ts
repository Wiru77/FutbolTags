...
@Component({
  selector: 'app-archivos-categorias',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-categorias.component.html',
  styleUrl: './archivos-categorias.component.css',
})
export class ArchivosCategoriasComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.categorias.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_categorias(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.categorias = this.jugador.categorias.filter(
