...
@Component({
  selector: 'app-archivos-intdeportiva',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-intdeportiva.component.html',
  styleUrl: './archivos-intdeportiva.component.css',
})
export class ArchivosIntdeportivaComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.intdeportiva.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_intdeportiva(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.intdeportiva = this.jugador.intdeportiva.filter(
