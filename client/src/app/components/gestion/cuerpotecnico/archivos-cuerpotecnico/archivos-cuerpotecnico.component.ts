...
@Component({
  selector: 'app-archivos-cuerpotecnico',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-cuerpotecnico.component.html',
  styleUrl: './archivos-cuerpotecnico.component.css',
})
export class ArchivosCuerpotecnicoComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.cuerpotecnico.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_cuerpotecnico(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.cuerpotecnico = this.jugador.cuerpotecnico.filter(
