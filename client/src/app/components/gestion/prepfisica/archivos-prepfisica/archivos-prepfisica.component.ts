...
@Component({
  selector: 'app-archivos-prepfisica',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-prepfisica.component.html',
  styleUrl: './archivos-prepfisica.component.css',
})
export class ArchivosPrepfisicaComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.prepfisica.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_prepfisica(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.prepfisica = this.jugador.prepfisica.filter(
