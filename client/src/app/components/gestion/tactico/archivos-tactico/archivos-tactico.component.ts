...
@Component({
  selector: 'app-archivos-tactico',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-tactico.component.html',
  styleUrl: './archivos-tactico.component.css',
})
export class ArchivosTacticoComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.tactico.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_tactico(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.tactico = this.jugador.tactico.filter(
