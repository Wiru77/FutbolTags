...
@Component({
  selector: 'app-archivos-casaclub',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-casaclub.component.html',
  styleUrl: './archivos-casaclub.component.css',
})
export class ArchivosCasaclubComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.casaclub.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_casaclub(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.casaclub = this.jugador.casaclub.filter(
