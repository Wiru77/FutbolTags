...
@Component({
  selector: 'app-archivos-prensa',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-prensa.component.html',
  styleUrl: './archivos-prensa.component.css',
})
export class ArchivosPrensaComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.prensa.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_prensa(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.prensa = this.jugador.prensa.filter(
