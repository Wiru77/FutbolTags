...
@Component({
  selector: 'app-archivos-porteros',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-porteros.component.html',
  styleUrl: './archivos-porteros.component.css',
})
export class ArchivosPorterosComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.porteros.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_porteros(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.porteros = this.jugador.porteros.filter(
