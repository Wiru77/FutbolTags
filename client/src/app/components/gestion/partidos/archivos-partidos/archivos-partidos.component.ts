...
@Component({
  selector: 'app-archivos-partidos',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-partidos.component.html',
  styleUrl: './archivos-partidos.component.css',
})
export class ArchivosPartidosComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.partidos.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_partidos(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.partidos = this.jugador.partidos.filter(
