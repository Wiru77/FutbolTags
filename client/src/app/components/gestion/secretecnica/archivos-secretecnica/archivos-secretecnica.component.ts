...
@Component({
  selector: 'app-archivos-secretecnica',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-secretecnica.component.html',
  styleUrl: './archivos-secretecnica.component.css',
})
export class ArchivosSecretecnicaComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.secretecnica.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_secretecnica(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.secretecnica = this.jugador.secretecnica.filter(
