...
@Component({
  selector: 'app-archivos-visorias',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-visorias.component.html',
  styleUrl: './archivos-visorias.component.css',
})
export class ArchivosVisoriasComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.visorias.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_visorias(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.visorias = this.jugador.visorias.filter(
