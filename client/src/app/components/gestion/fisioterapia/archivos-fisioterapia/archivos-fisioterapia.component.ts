...
@Component({
  selector: 'app-archivos-fisioterapia',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-fisioterapia.component.html',
  styleUrl: './archivos-fisioterapia.component.css',
})
export class ArchivosFisioterapiaComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.fisioterapia.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_fisioterapia(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.fisioterapia = this.jugador.fisioterapia.filter(
