...
@Component({
  selector: 'app-archivos-deshumano',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-deshumano.component.html',
  styleUrl: './archivos-deshumano.component.css',
})
export class ArchivosDeshumanoComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.deshumano.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_deshumano(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.deshumano = this.jugador.deshumano.filter(
