...
@Component({
  selector: 'app-archivos-administracion',
  standalone: true,
  imports: [NavComponent, RouterModule, FormsModule, CommonModule],
  templateUrl: './archivos-administracion.component.html',
  styleUrl: './archivos-administracion.component.css',
})
export class ArchivosAdministracionComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    this._jugadorService
      ...
      .subscribe({
        next: (res) => {
          this.jugador.administracion.push(res.data); // añadir al array, no reemplazar
  ...
  eliminarArchivo(archivoId: string): void {
    this._jugadorService
      .eliminar_archivo_administracion(this.jugadorId, archivoId, this.token)
      .subscribe({
        next: () => {
          this.jugador.administracion = this.jugador.administracion.filter(
